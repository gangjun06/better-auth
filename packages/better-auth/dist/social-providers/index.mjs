...
const kakao = (options) => {
  return {
    ...
    async createAuthorizationURL({ state, scopes, codeVerifier, redirectURI }) {
      ...
      const _scopes = options.disableDefaultScope ? [] : ["openid", "profile_nickname", "profile_image", "account_email"];
      options.scope && _scopes.push(...options.scope);
      scopes && _scopes.push(...scopes);
      const url = await createAuthorizationURL({
        id: "kakao",
      ...
      options.prompt && url.searchParams.set("prompt", options.prompt);
      options.service_terms && url.searchParams.set("service_terms", options.service_terms);
      return url;
    },
    validateAuthorizationCode: async ({ code, codeVerifier, redirectURI }) => {
      return validateAuthorizationCode({
    ...
    async getUserInfo(token) {
      ...
      if (token.idToken && !token.accessToken) {
        ...
        const userMap2 = await options.mapProfileToUser?.(user);
        return {
          user: {
            id: user.sub,
            name: user.nickname,
      ...
      const data = { ...response, type: "default" };
      ...
      return {
        user: {
          ...
          image: data.properties?.profile_image || data.kakao_account?.profile?.profile_image_url,
...
export { LANG, SocialProviderListEnum, apple, discord, dropbox, facebook, getApplePublicKey, github, gitlab, google, kakao, linkedin, microsoft, reddit, roblox, socialProviderList, socialProviders, spotify, tiktok, twitch, twitter, vk };
